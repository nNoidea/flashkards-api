...
const up = async (knex: Knex) => {
    await knex.schema.createTable(data.tables.scoreboard, (table) => {
        ...
        table.foreign("user_id", "fk_scoreboard_user").references(`${data.tables.user}.id`).onDelete("CASCADE");
        table.foreign("card_id", "fk_card_user").references(`${data.tables.card}.id`).onDelete("CASCADE");
    });
};

const down = (knex: Knex) => {
    return knex.schema.dropTableIfExists(data.tables.scoreboard);
};
